const DitherLib = (() => {
  ...
  const errDiff = (d, c, p) => {
    ...
    for (let y = 0; y < h; y++) {
      ...
      for (let x = xs; x !== xe; x += dir) {
        ...
        const er = (r - cl.r) * c.diffusion;
        const eg = (g - cl.g) * c.diffusion;
        const eb = (b - cl.b) * c.diffusion;
        data[i] = cl.r;
        data[i + 1] = cl.g;
        data[i + 2] = cl.b;
        for (let j = 0; j < m.length; j++) {
          ...
          if (nx >= 0 && nx < w && ny < h) {
            const ni = (ny * w + nx) * 4;
            data[ni] = clamp(data[ni] + er * w);
            data[ni + 1] = clamp(data[ni + 1] + eg * w);
            data[ni + 2] = clamp(data[ni + 2] + eb * w);
          }
        }
      }
    }
  ...
  const img = (im, uc = {}) => {
    try {
      ...
      const p =
        c.palette && P[c.palette] ? P[c.palette] : genPal(c.levels, c.isColor);
      engine(id, c, p);
  ...
  const gif = async (g, uc = {}) => {
    try {
      const c = valid(
        typeof uc === "object" && !g.hasAttribute("dither") ? uc : parse(g),
      );
      const cv = document.createElement("canvas");
      const cx = cv.getContext("2d");
      if (!cx) return null;
      cv.width = g.naturalWidth || g.width;
      cv.height = g.naturalHeight || g.height;
      const p =
        c.palette && P[c.palette] ? P[c.palette] : genPal(c.levels, c.isColor);
      let run = true;
      let last = 0;
      ...
      const proc = (t) => {
        ...
        if (t - last >= fi) {
          cx.filter = `contrast(${c.contrast}) brightness(${1 + c.brightness})`;
          cx.drawImage(g, 0, 0, cv.width, cv.height);
          cx.filter = "none";
          const id = cx.getImageData(0, 0, cv.width, cv.height);
          engine(id, c, p);
          cx.putImageData(id, 0, 0);
  ...
  const init = async (s = "img[dither], video[dither]") => {
    ...
    for (let e of els) {
      if (e.tagName === "VIDEO") {
        ...
        if (isGif) {
          if (e.complete) {
            proms.push(Promise.resolve(gif(e)));
          } else {
            proms.push(
              new Promise((r) => {
                e.onload = () => r(gif(e));
  ...
  return {
    ...
    applyToGif: gif,
    palettes: P,
    algorithms: Object.keys(M).concat(["ordered", "bayer", "random", "noise"]),
    version: "2.0.0",
  };
})();
