const DitherLib = (() => {
  ...
  const gif = (g, oc, uc = {}) => {
    try {
      ...
      const cx = oc.getContext("2d");
      if (!cx) return null;
      oc.width = g.naturalWidth || g.width;
      oc.height = g.naturalHeight || g.height;
      const p =
        c.palette && P[c.palette] ? P[c.palette] : genPal(c.levels, c.isColor);
      let run = true;
      let last = 0;
      ...
      const proc = (t) => {
        ...
        if (t - last >= fi) {
          cx.filter = `contrast(${c.contrast}) brightness(${1 + c.brightness})`;
          cx.drawImage(g, 0, 0, oc.width, oc.height);
          cx.filter = "none";
          const id = cx.getImageData(0, 0, oc.width, oc.height);
          engine(id, c, p);
          cx.putImageData(id, 0, 0);
  ...
  return {
    ...
    applyToGif: (g, uc = {}) => {
      const cv = document.createElement("canvas");
      cv.className = g.className;
      cv.style.cssText = g.style.cssText;
      g.parentNode.insertBefore(cv, g);
